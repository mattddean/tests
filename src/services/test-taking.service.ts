...
import type { Database } from "@/db/live";
import type { QuestionType, TestPermission } from "@/lib/tests/model";
...
function getOrCreateResponse(db: TestsDb, testId: string, userId: string) {
  return Effect.gen(function* () {
    ...
    return yield* requirePresent(
      created,
      new ResponseNotFound({ message: "Failed to create response" }),
    );
  });
}
...
function saveAnswerRecord(
  ...
  return Effect.gen(function* () {
    ...
    yield* requirePresent(
      choice,
      new ChoiceNotFound({ message: "Choice does not belong to question" }),
    );

    const response = yield* getOrCreateResponse(db, testId, userId);
    if (response.status === "submitted") {
      return yield* Effect.fail(
        new ResponseAlreadySubmitted({ message: "Response already submitted" }),
      );
    }
    ...
    const [existingAnswer] = yield* db
      ...
      .from(responseAnswer)
      .where(
        and(eq(responseAnswer.responseId, response.id), eq(responseAnswer.questionId, questionId)),
      )
      .limit(1);
    ...
    if (existingAnswer) {
      yield* db
        ...
        .set({ choiceId, updatedAt: new Date() })
        .where(eq(responseAnswer.id, existingAnswer.id));
    } else {
      yield* db
        .insert(responseAnswer)
        .values({ id: createId(), responseId: response.id, questionId, choiceId });
    }
        ...
        Effect.gen(function* () {
          ...
          if (permission === "owner" || permission === "editor") {
            ...
            if (currentTest.status !== "published") {
              return yield* Effect.fail(
                new TestMustBePublished({ message: "Only published tests can collect responses" }),
              );
            }
          }
      ...
      submitResponse: (testId: string, userId: string) =>
        Effect.gen(function* () {
          ...
          if (resolvedTest.status !== "published") {
            return yield* Effect.fail(
              new TestMustBePublished({ message: "Only published tests can be submitted" }),
            );
          }
          ...
          if (missingRequired) {
            return yield* Effect.fail(
              new RequiredQuestionsIncomplete({
                message: "Complete all required questions before submitting",
              }),
            );
          }
