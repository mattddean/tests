...
import { parseSaveAnswerInput, parseSubmitResponseInput } from "@/domains/tests/schema";
...
export const saveAnswerAction = createServerFn({ method: "POST" })
  .inputValidator(parseSaveAnswerInput)
  .handler(({ data }) =>
    runServerEffect(
...
export const submitResponseAction = createServerFn({ method: "POST" })
  .inputValidator(parseSubmitResponseInput)
  .handler(({ data }) =>
    runServerEffect(
      withCurrentUser((userId) =>
